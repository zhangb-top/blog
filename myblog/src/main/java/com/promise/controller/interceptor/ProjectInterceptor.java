...
public class ProjectInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if ("OPTIONS".equals(request.getMethod()) || !"home".equals(request.getHeader("router"))) {
            return true;
        }
        // 获取请求头的token数据
        ...
        if (token != null) {
            ...
            if (!TokenUtils.verify(token)) {
                response.getWriter().write(objectMapper.writeValueAsString(new UserResult(Code.TOKEN_ER,
                        "token无效")));
                return false;
            } else return true;
        } else {
